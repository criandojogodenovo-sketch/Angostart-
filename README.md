...
| 🤝 Afiliados com elegibilidade | **Vendedor: 5+ vendas pagas · Cliente: 2+ compras pagas** (Fase 10, modelo Shopee) — verificação automática do histórico; sem requisito → 403 com mensagem «Necessitas de X…» |
| 🔗 Links de afiliado | Código capturado por `?ref=AFG-XXXXXX` (**30 dias configuráveis** em localStorage — `NEXT_PUBLIC_AFFILIATE_ATTRIBUTION_DAYS`, componente `RefCapture`), pré-preenchido no checkout; campo «Código de afiliado» no registo (`users.referred_by`); botão «Copiar link de afiliado» nos produtos |
| 📢 Sub-ID / campanhas (Fase 10) | Links por canal `?ref=AFG-XXXX&sub=instagram` — o canal fica registado na encomenda (`orders.affiliate_sub_id`) e na comissão (`affiliate_earnings.sub_id`); **relatório de vendas por canal** no painel de afiliado |
| 📈 Escalão automático | 50+ comissões recebidas → comissão sobe de 10% para **15%** (`resolveAffiliatePercent`, atualiza a BD); barra de progresso «Faltam X vendas para ganhares 15%» |
| ⚡ Gerador em massa (Fase 10) | Painel do afiliado: cola vários URLs/IDs de produtos → gera todos os links de afiliado de uma vez, com botão **«Copiar todos»** (modelo Shopee batch) |
